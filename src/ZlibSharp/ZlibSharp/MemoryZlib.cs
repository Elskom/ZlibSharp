...
public static unsafe class MemoryZlib
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static uint Decompress(byte[] Source, byte[] Dest)
    {
        return Decompress(Source.AsSpan(), Dest.AsSpan());
    }
    
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static uint Decompress(string SourcePath, byte[] Dest)
    {
        var Source = File.ReadAllBytes(SourcePath);
        
        return Decompress(Source, Dest, out _);
    }
    
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static uint Decompress(Span<byte> Source, Span<byte> Dest)
    {
        return Decompress(Source, Dest, out _);
    }
    
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static uint Decompress(Span<byte> Source, Span<byte> Dest, out uint Adler32)
    {
        return ZlibHelper.Decompress(Source, Dest, out Adler32);
    }
