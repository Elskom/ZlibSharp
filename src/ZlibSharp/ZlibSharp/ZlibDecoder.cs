...
public class ZlibDecoder
{
    ...
    /// </returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public OperationStatus Decompress(string sourcePath, Span<byte> dest, out uint bytesWritten, out uint bytesRead, out uint hash)
        => this.Decompress(File.ReadAllBytes(sourcePath), dest, out bytesWritten, out bytesRead, out hash);
    ...
    /// </returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public OperationStatus Decompress(ReadOnlySpan<byte> source, Span<byte> dest, out uint bytesWritten, out uint bytesRead, out uint hash)
        => this.DecompressCore(source, dest, out bytesWritten, out bytesRead, out hash);

    [ExcludeFromCodeCoverage]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private OperationStatus DecompressCore(ReadOnlySpan<byte> source, Span<byte> dest, out uint bytesWritten, out uint bytesRead, out uint hash)
    {
    ...
    {
        status = this.DecompressCore(source, dest, out bytesWritten, out bytesRead, out hash);
